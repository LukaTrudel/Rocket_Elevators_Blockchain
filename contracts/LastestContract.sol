...
contract NFT is ERC721Enumerable, Ownable{
  ...
  constructor(
    ...
    address _rocketToken
  ) ERC721(_name, _symbol) {
    setBaseURI(_initBaseURI);
    setNotRevealedURI(_initNotRevealedUri);
    setRocketTokenURI(_rocketToken);
  }
  ...
  // public
  function mintWithToken(uint256 _mintAmount) external payable {
    require(!paused, "the contract is paused");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
    require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
    require(block.timestamp <= saleEnd, "Sales Ended");

    uint256 balance = IERC20(rocketTokenAddress).balanceOf(msg.sender);
    require(balance >= _mintAmount, "You don't have enought token"); 
  ...
  function setRocketTokenURI(address _rocketToken) public onlyOwner {
    rocketTokenAddress = _rocketToken;
  }
}
